...
st.set_page_config(
    page_title="Phân tích hashtag và bài hát",
    page_icon=":bar_chart:",
    layout="wide",
...
def hashtag_song_analysis(cleaned_video_info_df):
    ...
    with st.spinner("Analyzing data..."):
        filtered_data = cleaned_video_info_df[(cleaned_video_info_df['createTime'] >= start_date) & (
            cleaned_video_info_df['createTime'] <= end_date)]
    ...
    if not filtered_data.empty:
        with st.expander("🔥 Most Used Hashtags", expanded=True):
            ...
            if not all_hashtags.empty:
                ...
                fig.update_layout(xaxis_title="Usage Count",
                                  yaxis_title="Hashtag", template="plotly_white")
                st.plotly_chart(fig, use_container_width=True)
                if st.checkbox("Show Hashtag Table"):
                    st.dataframe(top_hashtags)
            else:
                st.markdown(
                    '<p style="color:#e74c3c;">⚠️ No hashtags found.</p>', unsafe_allow_html=True)

        with st.expander("🎵 Most Used Songs", expanded=True):
            music_counts = filtered_data['music.authorName'].value_counts(
            ).reset_index()
            music_counts.columns = ['Music Author', 'Count']
            top_music = music_counts.head(top_k)
            if not top_music.empty:
                fig = px.bar(top_music, x='Count', y='Music Author', orientation='h',
                             title="🎶 Most Used Songs", color='Count', color_continuous_scale='viridis')
                fig.update_layout(
                    xaxis_title="Usage Count", yaxis_title="Music Author", template="plotly_white")
                st.plotly_chart(fig, use_container_width=True)
                if st.checkbox("Show Song Table"):
                    st.dataframe(top_music)
            else:
                st.markdown(
                    '<p style="color:#e74c3c;">⚠️ No songs found.</p>', unsafe_allow_html=True)
    else:
        st.markdown(
            '<p style="color:#e74c3c;">⚠️ No data available for this range.</p>', unsafe_allow_html=True)
