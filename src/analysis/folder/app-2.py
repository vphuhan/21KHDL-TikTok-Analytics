...
selected_section = st.sidebar.radio("🔍 Select Analysis", ["📈 Hashtag Engagement", "🎬 Video Performance", "📈 Hashtag Insights"])

# 👉 General Filters
with st.sidebar.expander("⚙️ Filters", expanded=True):
    selected_metric = st.selectbox("📊 Select Metric", ["Views", "Likes", "Comments", "Shares"], index=0)
    metric_map = {
...
# 🎮 Video Performance Insights
    ...
    with col1:
        ...
        st.plotly_chart(plot_video_duration_vs_views(df, metric_col), use_container_width=True)
    with col2:
        st.subheader(f"📌 Engagement vs. {selected_metric}")
        st.plotly_chart(plot_video_duration_vs_engagement(df, metric_col), use_container_width=True)
    ...
    with col1:
        ...
        st.plotly_chart(plot_posting_time_vs_views(df, metric_col), use_container_width=True)
    with col2:
        st.subheader("📅 Posting Day vs. Engagement")
        st.plotly_chart(plot_posting_day_vs_engagement(df, metric_col), use_container_width=True)
...
# 📊 Hashtag Insights
    ...
    with st.sidebar.expander("📌 Hashtag Filters", expanded=True):
        ...
        time_agg = st.radio("Choose Time Grouping:", ["Daily", "Weekly", "Monthly"])
    
    col1, col2 = st.columns(2)
    with col1:
        st.subheader(f"📊 Hashtag Engagement Insights ({selected_metric})")
        st.plotly_chart(plot_improved_top_hashtags(analyze_hashtag_engagement(df, top_n, metric_col)), use_container_width=True)
    with col2: 
        # Hashtag Count Analysis
        st.subheader(f"📌 Hashtag Count Effect on {selected_metric}")
        st.plotly_chart(plot_interactive_hashtag_analysis(analyze_hashtag_count_effect(df, metric_col), get_top_hashtags_by_group(df, metric_col), metric_col), use_container_width=True)

    st.markdown("---")
    
    st.subheader(f"📊 Hashtag Trends Over Time ({selected_metric})")
    st.plotly_chart(plot_hashtag_trends(df, time_agg, top_n, metric_col), use_container_width=True)

    # Hashtag Count vs. Engagement Scatter
    st.subheader(f"📌 Hashtag Count vs. {selected_metric}")
    st.plotly_chart(plot_hashtag_count_vs_engagement(df, metric_col), use_container_width=True)
