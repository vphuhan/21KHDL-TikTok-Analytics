...
def create_bar_chart(data, metric, title, color_scale, y_label):
    ...
    fig = px.bar(
        ...
        text=data[metric].apply(lambda x: f'{x:,.1f}' if metric == 'engagement_rate' else f'{x:,}'),
        height=500
    )
...
def top_users(df):
    ...
    with col2:
        chart_option = st.selectbox("Chọn loại biểu đồ", list(CHART_OPTIONS.keys()))
    with col3:
        sort_order = st.radio("Thứ tự sắp xếp", ["Giảm dần", "Tăng dần"], index=0)

    # Xử lý dữ liệu
    if chart_option == "Tỷ lệ tương tác":
        df['engagement_rate'] = df['stats.heart'] / df['stats.videoCount'].replace(0, 1)
    ...
    with st.spinner(f"Đang tạo hình ảnh cho Top {top_n} người dùng..."):
        with tab_bar:
            ...
            bar_fig = create_bar_chart(top_data, metric, title, color_scale, y_label)
    ...
    with st.expander("Xem dữ liệu chi tiết", expanded=False):
        st.dataframe(
            top_data.style.format({metric: "{:,.2f}" if metric == 'engagement_rate' else "{:,}"}),
...
def main():
    ...
    if not df.empty:
        ...
        st.warning("Vui lòng tải lên hoặc cung cấp dữ liệu người dùng hợp lệ để phân tích.")
